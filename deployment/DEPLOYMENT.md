...
- `STAGING_DATABASE_URL` / `PRODUCTION_DATABASE_URL` — direct Postgres connection strings (not the Supabase REST URL) with migration privileges, one per GitHub Environment (`staging` / `production`). Used by `.github/workflows/run-db-migrations.yml`, the manual `workflow_dispatch` job that runs `alembic upgrade head` against the selected environment. Configure these on the matching GitHub Environment (not as bare repo secrets) so the `production` value is only ever exposed to a run that has passed that Environment's required-reviewer gate.
