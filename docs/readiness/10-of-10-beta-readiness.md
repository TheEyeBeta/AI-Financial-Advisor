...
| Branch ruleset applied on `main`/`staging` (PR required, 1 approval, dismiss stale, resolved threads, up-to-date, mandatory checks, no force-push/deletion, no admin bypass) | AUTOMATED TEST PASSED (live probes) | Ruleset `Production branch protection` (ID `19108544`) active on `main`+`staging`; direct push/force-push/delete all rejected (`GH013`) — `docs/evidence/platform/github-ruleset-20260717.md`. 2026-07-22: added the three `readiness-controls.yml` checks to the required list (12 total) — `docs/evidence/platform/github-ruleset-20260722.md`. `release-verification.yml` cannot be a PR gate (post-deploy only, see `CI_GUIDELINES.md` §3.8). |
