-- Complete Fix: Trigger Function + RLS Policy
...
-- Create policy that allows INSERT (for trigger function)
-- Note: SECURITY DEFINER should bypass RLS, but this ensures compatibility
CREATE POLICY "Service role can insert user profiles" ON public.users
    FOR INSERT 
    WITH CHECK (true);

-- Success message
SELECT '✅ Trigger function and RLS policy fixed!' AS status;
