...
export function useSendChatMessage() {
  ...
  return useMutation({
    mutationFn: async ({ message, response }: { message: string; response?: string }) => {
      ...
      // Save user message
      await chatApi.addMessage(userId, 'user', message);
      
      // Get AI response (either provided or from Python backend)
      const aiResponse = response || await pythonApi.getChatResponse(message, userId);
      
      // Save AI response
      await chatApi.addMessage(userId, 'assistant', aiResponse);
