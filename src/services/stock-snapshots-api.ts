...
const stockCache = {
  ...
  setMainList(data: StockSnapshot[]): void {
    ...
    data.forEach((snapshot) => {
      this.tickers.set(snapshot.ticker.toUpperCase(), { data: snapshot, timestamp: Date.now() });
      if (snapshot.company_name) {
        this.companyNames.set(snapshot.company_name.toLowerCase(), { data: snapshot, timestamp: Date.now() });
      }
    });
